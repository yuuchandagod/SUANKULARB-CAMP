...
class PreferencesData:
    def __init__(self, pref: _Union[str, dict], /, *, mode: str = 'file') -> None:
        ...
        if isinstance(self.__pref, str):
            ...
            if self.__format and self.__format in ['json', 'toml', 'xml', 'yaml']:
                self.__pref_dict = self.__format_parser[self.__format](
                    self.__pref, mode=self.__mode)
                self.logger.debug('Extension correct: ' + self.__format)

            else:
                try:
                    self.__pref_dict = self.parseJSON(
                        self.__pref, mode=self.__mode)
                    self.__format = 'json'
                    self.logger.info(
                        'Preferences format is JSON. Using JSON Loader.')
                    self.logger.info('Loaded JSON as dictionary')
                except ValueError or _json.decoder.JSONDecodeError:
                    pass

                try:
                    self.__pref_dict = self.parseTOML(
                        self.__pref, mode=self.__mode)
                    self.__format = 'toml'
                    self.logger.info(
                        'Preferences format is TOML. Using TOML Loader.')
                    self.logger.info('Loaded TOML as dictionary')
                except ValueError or _toml.decoder.TomlDecodeError:
                    pass

                try:
                    self.__pref_dict = self.parseXML(
                        self.__pref, mode=self.__mode)
                    self.__format = 'xml'
                    self.logger.info(
                        'Preferences format is XML. Using XML Tree Loader.')
                    self.logger.info('Loaded XML as dictionary')
                except ValueError or _xml.parsers.expat.ExpatError:
                    pass

                try:
                    self.__pref_dict = self.parseYAML(
                        self.__pref, mode=self.__mode)
                    self.__format = 'yaml'
                    self.logger.info(
                        'Preferences format is YAML. Using YAML Loader.')
    ...
    def writePreferences(self, pref_dict=None, pref_file_name=None, file_format=None) -> bool:
        ...
        if not isinstance(__pref_file_name, str):
            raise TypeError(
                'File name only supports \'str\' not \'' + str(type(__pref_file_name)) + '\'')

        if __file_format and __file_format in ['json', 'toml', 'xml', 'yaml']:
            self.__pref_dict = self.__format_writer[self.__format](
                __pref, __pref)
            return True
        return False
...
if __name__ == '__main__':
    ...
    s = PreferencesData.parseYAML(
        'hello:\n    - image\n    - command', strict_validate=False)
    ptr = {'x': {'hello': 'image', 'tt': 'command',
                 'qq': ['jkdashifk', 'dghsdhfj']}}
